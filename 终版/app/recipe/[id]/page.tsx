...
import { ChevronLeft, Clock, Flame, Users, Heart, Share2, ShoppingCart, RefreshCw, Search } from 'lucide-react';
...
export default function RecipeDetailPage() {
  ...
  const handleFindSubstitute = (ingredient: string) => {
    setSelectedIngredient(ingredient);
    setSubstituteModalOpen(true);
  };

  const handleSelectSubstitute = (substituteName: string, adjustedSteps: string[]) => {
    if (recipe) {
      setRecipe({
        ...recipe,
        steps: adjustedSteps,
        required_ingredients: recipe.required_ingredients.map(ing =>
          ing === selectedIngredient ? substituteName : ing
        ),
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          <div className="flex flex-col gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recipe.missing_ingredients.map((ingredient, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-1 px-3 py-1 bg-[#FFE8CC] text-[#CC7A00] rounded-full text-sm"
                      >
            ...
            {/* Nutrition Analysis */}
            <NutritionAnalysis
              ingredients={[...recipe.required_ingredients, ...recipe.optional_ingredients]}
            />

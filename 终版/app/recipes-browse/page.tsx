...
export default function RecipesBrowsePage() {
  ...
  // Get all unique cuisines and taste tags from database
  const cuisines = Array.from(new Set(recipesDatabase.map(r => r.cuisine))).sort();
  const tasteTags = Array.from(new Set(recipesDatabase.flatMap(r => r.taste_tags))).sort();
  ...
  // Apply taste filter
  if (tasteFilter !== 'all') {
    filteredRecipes = filteredRecipes.filter(recipe => recipe.taste_tags.includes(tasteFilter));
  }
    ...
    <div className="flex flex-col h-full w-full bg-[var(--color-bg)] overflow-hidden">
      ...
      <div className="flex flex-col flex-1 gap-4 px-6 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          {(difficultyFilter !== 'all' || cuisineFilter !== 'all' || tasteFilter !== 'all' || maxTime < 120) && (
            <button
              onClick={() => {
